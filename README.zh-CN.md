...
> **junkai 定制版本：** 此仓库是带有 TUI 容量信息功能的下游 Hermes
> 版本。**不要使用下面 Nous 的一行安装命令**：它只会安装上游 Hermes，无法
> 启用本仓库的定制。请克隆此仓库后执行：
>
> ```bash
> git clone https://github.com/wo52616111/hermes-agent-jk.git
> cd hermes-agent-jk
> ./scripts/hermes-local-install
> hermes
> ```
>
> 安装器会将本仓库接到正常的 `hermes` 命令，并默认启动 TUI。前置条件、验证
> 方式以及下游更新流程请见 [`README.patched.md`](README.patched.md)。

## 本仓库定制功能

- **TUI 用量与额度信息**：独立容量行展示 context 使用量、服务商的 `5h` / `7d`
  配额窗口及可用时的重置倒计时；OpenAI Codex、Anthropic、OpenCode Go 会在完成
  一轮后刷新额度，刷新失败时保留上一次成功的快照。
- **默认 TUI 与定制外观**：直接运行 `hermes` 即启动 Ink TUI，安装
  `jk-spaceduck` 皮肤，并在启动 Banner 标记 “Customized by junkai”。
- **草稿暂存快捷键**：`Ctrl-S` 暂存完整 composer 草稿（包括多行输入、图片/大粘贴
  token）；执行 slash command 或 `!cmd` 后再次按下可恢复这一个内存暂存槽。
- **理解 downstream 的更新路径**：`hermes update` 先同步本 fork 最新
  `origin/main`，再将定制提交栈 rebase 到 Nous upstream；运行中的 TUI 可以继续
  使用已加载版本，重启后使用更新后的构建。
...
> 此一行安装命令**不包含**本仓库的 `junkai` 定制；分享此 fork 时请使用上方的
> 定制版本安装步骤。
